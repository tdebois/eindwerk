...
if (!isset($_SESSION['emailvergeten'])) {
    $_SESSION['emailvergeten'] = "";
}
?>

<h1>Wachtwoord vergeten?</h1>
<h1>Vul uw e-mailadres in en uw wachtwoord wordt opgestuurt.</h1>

<form id="form" name="form" method="post" action="../gebruiker/wachtwoordversturen.php">
    <p><label for="email">Emailadres</label>
    <input type="text" name="email" id="email" value="<?php echo $_SESSION['emailvergeten'];?>" /></p>

    <p>
        <input type="submit" name="bevestig" id="bevestig" value="Verzend het wachtwoord" />
    </p>

</form>

<?php
if (isset($_SESSION['error']['emailvergeten']) ) {
    echo $_SESSION['error']['emailvergeten']."<br>";
}

if (isset($_GET['email']) && $_GET['email'] == 'succes') {
    echo "Er werd een e-mail verzonden met uw inloggegevens";
}

if (isset($_GET['email']) && $_GET['email'] == 'verkeerd') {
    echo "Gelieve het emailadres in te vullen waarmee u geregistreerd hebt";
}
